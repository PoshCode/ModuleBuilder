Describe "ResolveModuleManifest" {
    ...
    It "Should return the ModuleBuilder path when passed just a module folder" {
        ...
        $Expected | Should -Be ${Global:Test Root Path}\ModuleBuilder.psd1
    }

    It "Should return the ModuleBuilder path when passed a module folder and null" {
        $Expected = InModuleScope ModuleBuilder { ResolveModuleManifest ${Global:Test Root Path} $null }
        $Expected | Should -Be ${Global:Test Root Path}\ModuleBuilder.psd1
    }

    It "Should return the ModuleBuilder path when passed a relative module manifest" {
        $Expected = InModuleScope ModuleBuilder { ResolveModuleManifest ${Global:Test Root Path} ModuleBuilder.psd1 }
        $Expected | Should -Be ${Global:Test Root Path}\ModuleBuilder.psd1
    }

    It "Should return the ModuleBuilder path when passed an absolute module manifest" {
